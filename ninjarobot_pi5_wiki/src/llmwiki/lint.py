...
def trust_tier(metadata: dict[str, Any]) -> str:
    ...
    if any(
        isinstance(item, dict) and str(item.get("by", "")).startswith("human:") for item in events
    ):
...
def _profile_errors(config: Config, metadata: dict[str, Any], relative: str) -> list[Issue]:
    ...
    schema = json.loads(
        (config.root / "schemas/okf-v0.2-profile.schema.json").read_text(encoding="utf-8")
    )
    issues = []
...
def _semantic_issues(
    ...
    *,
    ...
    if state == "missing":
        issues.append(
            Issue(
                ...
                "Page requiring semantic review has no page-level review",
                relative,
            )
        )
    elif state == "stale":
        issues.append(
            Issue(
                ...
                "Semantic review no longer matches the page content or evidence",
                relative,
            )
        )
    elif state == "incomplete":
        issues.append(
            Issue(severity, "semantic-review-incomplete", "Semantic review is incomplete", relative)
        )
    elif state == "concerns":
        issues.append(
            Issue(
                ...
                "Semantic review records unresolved concerns",
                relative,
            )
        )

    concern_checks = sorted(
        name for name, value in checks.items() if value in {"concern", "not_inspected"}
    )
    ...
    has_image_evidence = any(
        records_by_id.get(source_id, {}).get("kind") == "image" for source_id in source_ids
    ...
    if (
        ...
        issues.append(
            Issue(
                ...
                "Image-based evidence requires actual visual inspection before "
                "semantic review can pass",
                relative,
            )
        )
    return issues
...
def lint_bundle(
    bundle_root: Path, config: Config, *, include_index_check: bool = True, strict: bool = False
) -> list[Issue]:
    issues: list[Issue] = []
    try:
    ...
    for path in bundle_root.rglob("*"):
        if path.is_symlink():
            ...
            issues.append(
                Issue(
                    "error", "symlink", "Symlinks are not allowed inside the OKF bundle", relative
                )
            )
    ...
    if not root_index.is_file():
        issues.append(
            Issue("error", "missing-root-index", "wiki/index.md is required", "wiki/index.md")
        )
    else:
        try:
    ...
    for path in pages:
        ...
        for kind in detected_secret_kinds(document.body):
            issues.append(
                Issue(
                    "warning",
                    "possible-secret",
                    f"Possible {kind} found; review before sharing",
                    relative,
                )
            )
            ...
            if not metadata.get(field):
                issues.append(
                    Issue(
                        "warning",
                        "profile-field",
                        f"Missing recommended profile field: {field}",
                        relative,
                    )
                )
        ...
        if page_type and config.allowed_types and page_type not in config.allowed_types:
            issues.append(
                Issue(
                    "suggestion",
                    "unknown-type",
                    f"Unknown local page type is still valid OKF: {page_type}",
                    relative,
                )
            )
        ...
        for label in sorted(references - definitions):
            issues.append(
                Issue(
                    ...
                    f"Footnote '{label}' has no definition",
                    relative,
                )
            )
        for label in sorted(references | definitions):
            if label not in source_ids:
                issues.append(
                    Issue(
                        "error",
                        "citation-source",
                        f"Footnote '{label}' has no matching sources[].id",
                        relative,
                    )
                )
            ...
            if source_id not in references:
                ...
                issues.append(
                    Issue(
                        severity,
                        "unused-source",
                        f"Source '{source_id}' is not cited in the body",
                        relative,
                    )
                )
        for item in metadata.get("sources", []):
            ...
            if source_id and record is None:
                issues.append(
                    Issue(
                        "error",
                        "unknown-source",
                        f"No catalog record exists for source '{source_id}'",
                        relative,
                    )
                )
                ...
                if not page_hash:
                    ...
                    issues.append(
                        Issue(
                            ...
                            f"Source '{source_id}' has no content_hash",
                            relative,
                        )
                    )
                    ...
                    issues.append(
                        Issue(
                            ...
                            f"Source '{source_id}' uses an older or unknown content hash",
                            relative,
                        )
                    )
            ...
            if _broad_resource(resource):
                issues.append(
                    Issue(
                        "warning",
                        "broad-source-resource",
                        f"Use an immutable file revision or source URN instead of: {resource}",
                        relative,
                    )
                )
            try:
                ...
                issues.append(
                    Issue(
                        "error",
                        "source-resource",
                        f"Source resource leaves the OKF bundle: {resource}",
                        relative,
                    )
                )
                continue
        ...
        if (
            ...
            and metadata.get("type") in {"Reference", "Analysis"}
            and not source_ids
        ):
            issues.append(
                Issue(
                    ...
                    "A stable Reference or Analysis page must list traceable sources",
                    relative,
                )
            )
        if (
            metadata.get("type") == "Reference"
            and metadata.get("resource")
            and _broad_resource(str(metadata["resource"]))
        ):
            issues.append(
                Issue(
                    "warning",
                    "broad-reference-resource",
                    "Reference resource should identify an exact revision or source URN",
                    relative,
                )
            )
        if source_ids:
            ...
            for line in _uncited_paragraphs(document.body):
                issues.append(
                    Issue(
                        uncited_severity,
                        "uncited-paragraph",
                        "Factual-looking paragraph has no source footnote; review it",
                        relative,
                        line,
                    )
                )
        ...
        for target, line in extract_wikilinks(document.body):
            issues.append(
                Issue(
                    "error",
                    "wikilink",
                    f"Use a standard Markdown link instead of [[{target}]]",
                    relative,
                    line,
                )
            )
    ...
    for path, document in parsed.items():
        ...
        for link in extract_links(full_text):
                ...
                if link.image:
                    issues.append(
                        Issue(
                            "warning",
                            "external-image",
                            "External image is not a portable, versioned bundle asset",
                            relative,
                            link.line,
                        )
                    )
            ...
            if not target.exists():
                issues.append(
                    Issue(
                        _severity(config, "broken_links", "error"),
                        "broken-link",
                        f"Missing target: {link.target}",
                        relative,
                        link.line,
                    )
                )
                ...
                if not link.label.strip():
                    issues.append(
                        Issue(
                            "warning",
                            "image-alt",
                            "Image link needs useful alt text",
                            relative,
                            link.line,
                        )
                    )
                try:
                    ...
                    issues.append(
                        Issue(
                            "error",
                            "image-location",
                            "Local image assets must stay under wiki/assets/",
                            relative,
                            link.line,
                        )
                    )
            ...
            if anchor and target.is_file() and target.suffix == ".md":
                if anchor not in anchors(target.read_text(encoding="utf-8")):
                    issues.append(
                        Issue(
                            _severity(config, "broken_anchors", "error"),
                            "broken-anchor",
                            f"Missing anchor '{anchor}' in {link.target}",
                            relative,
                            link.line,
                        )
                    )
    for path in pages:
        ...
        if inbound[path.resolve()] == 0:
            ...
            issues.append(
                Issue(
                    _severity(config, "orphan_pages", "warning"),
                    "orphan",
                    "Page has no inbound concept links",
                    relative,
                )
            )
            ...
            if asset.suffix.lower() not in IMAGE_EXTENSIONS:
                issues.append(
                    Issue(
                        "error",
                        "unsupported-asset",
                        "Only PNG and JPEG supporting assets are allowed",
                        relative,
                    )
                )
                continue
            if asset.resolve() not in referenced_assets:
                issues.append(
                    Issue(
                        "warning",
                        "orphan-asset",
                        "Asset is not referenced by a wiki page",
                        relative,
                    )
                )
            if "sources" not in asset.relative_to(assets_root).parts:
                issues.append(
                    Issue(
                        "warning",
                        "asset-location",
                        "Source renditions should live under wiki/assets/sources/",
                        relative,
                    )
                )
            ...
            if not record or record.get("kind") != "image":
                issues.append(
                    Issue(
                        "error",
                        "asset-provenance",
                        "Asset filename does not match a registered image source ID",
                        relative,
                    )
                )
                ...
                issues.append(
                    Issue(
                        ...
                        "Asset bytes do not match the registered sanitized rendition",
                        relative,
                    )
                )
    ...
    if include_index_check:
        for path in build_indexes(bundle_root, check=True):
            relative = f"wiki/{path.relative_to(bundle_root).as_posix()}"
            issues.append(
                Issue("warning", "index-drift", "Generated index is out of date", relative)
            )
    return sorted(
        issues,
        key=lambda item: (
            {"error": 0, "warning": 1, "suggestion": 2}[item.severity],
            item.path or "",
            item.code,
        ),
    )
...
def lint_project(config: Config, *, strict: bool = False) -> list[Issue]:
        ...
        if status == "missing":
            issues.append(
                Issue(
                    "error",
                    "missing-source",
                    f"Source file is missing: {record['path']}",
                    record["path"],
                )
            )
            ...
            issues.append(
                Issue(
                    _severity(config, "changed_sources", "error"),
                    "changed-source",
                    f"Source changed since registration: {record['id']}",
                    record["path"],
                )
            )
        ...
        if record.get("normalized_at"):
            if not derived_manifest.is_file():
                issues.append(
                    Issue(
                        "error",
                        "missing-derived-manifest",
                        "Normalized source is missing its derived manifest",
                        record["path"],
                    )
                )
            else:
                ...
                if manifest.get("source_hash") != record.get("content_hash"):
                    issues.append(
                        Issue(
                            ...
                            "Derived output was built from a different source hash",
                            record["path"],
                        )
                    )
                derived_content = derived_manifest.parent / "content.md"
                if not derived_content.is_file() or manifest.get("content_hash") != (
                    sha256_file(derived_content) if derived_content.is_file() else None
                ):
                    issues.append(
                        Issue(
                            "error",
                            "derived-content-mismatch",
                            "Derived content is missing or differs from its manifest",
                            record["path"],
                        )
                    )
                ...
                if rendition:
                    ...
                    if not rendition_path.is_file() or rendition.get("content_hash") != (
                        sha256_file(rendition_path) if rendition_path.is_file() else None
                    ):
                        issues.append(
                            Issue(
                                "error",
                                "derived-asset-mismatch",
                                "Sanitized rendition is missing or differs from its manifest",
                                record["path"],
                            )
                        )
        image_metadata = record.get("image") or {}
        if image_metadata.get("contains_gps_metadata") or image_metadata.get(
            ...
            issues.append(
                Issue(
                    ...
                    "Original image contains GPS or identifying EXIF metadata; "
        ...
        if record.get("original_uri") and _broad_resource(str(record["original_uri"])):
            issues.append(
                Issue(
                    "warning",
                    "broad-original-uri",
                    "Catalog URI should identify an immutable file revision",
                    record["path"],
                )
            )
            ...
            for kind in detected_secret_kinds(text):
                issues.append(
                    Issue(
                        "warning",
                        "possible-secret",
                        f"Possible {kind} in source; review privacy before remote use",
                        record["path"],
                    )
                )
    ...
    for folder in ("articles", "papers", "notes", "media"):
        ...
        if not root.exists():
            continue
        for path in sorted(
            item for item in root.rglob("*") if item.is_file() and not item.name.startswith(".")
        ):
            ...
            if path.suffix.lower() in config.allowed_extensions:
                issues.append(
                    Issue(
                        "suggestion",
                        "unregistered-source",
                        "Supported raw source is not registered",
                        relative,
                    )
                )
            else:
                issues.append(
                    Issue(
                        "suggestion",
                        "unsupported-source",
                        f"Unsupported raw source extension: {path.suffix or '<none>'}",
                        relative,
                    )
                )
    return sorted(
