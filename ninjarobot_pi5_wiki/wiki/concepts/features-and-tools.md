...
semantic_review:
  ...
  - Checkpoint claims distinguish software tests from physical hardware acceptance
    ...
    spending caps.
  target_hash: sha256:8e348f188eff44766736f73886e64a5092d6cdcc80fd6e5aa180b446e555b302
---
...
and tests to verify a specific feature or failure case before changing it.[^src-20260923-developmentguide-2]
...
they do not grant extra hardware permissions.[^src-20260923-ninjarobot-mcp-skill]
...
- **Tavily Search**: Hosted HTTPS transport using a bearer key from `SecretStore`; provides the `tavily_search` tool for bounded research snippets. Replacement keys are validated prior to saving.[^src-20260923-ninjarobot-mcp-skill]
- **Google Calendar**: Bundled external stdio MCP process reusing `calendar_oauth.authorize` and `GoogleCalendarBackend`; requests read-only event scope and exposes `list_today_events` with bounded title, start, end, and status fields. For Mac browsers, users forward port 8765 (`ssh -N -o ExitOnForwardFailure=yes -L 127.0.0.1:8765:127.0.0.1:8765 USER@ROBOT`). The wizard displays waiting updates, acknowledges the callback, validates an end-to-end bounded read-only events call, and cleans up candidate credential files on failure. It remains separate from native Calendar integration.[^src-20260923-ninjarobot-mcp-skill]
- **Notion**: Official hosted HTTPS transport using SDK OAuth on loopback callback port 8766; exposes `notion-search` and `notion-fetch` for workspace pages. Remote authorization requires SSH port forwarding (`ssh -L 8766:127.0.0.1:8766 USER@ROBOT`). Token refresh fails closed if reauthorization is needed.[^src-20260923-ninjarobot-mcp-skill]
...
Manual management uses `ninjarobot mcp list`, `mcp inspect SERVER`, and `mcp health SERVER`.[^src-20260923-ninjarobot-mcp-skill]
...
and keyboard shortcuts display on startup.[^src-20260923-developmentguide-2]
...
- **Agent View** (`/agent`): Provides a dedicated conversation pane, chat composer, a "Clear Message" action to clear visible chat items without resetting the backend session, special robot control toggles (A / B / X / Y) with high-contrast active/inactive styling, and audio control toggles (Web Mic, Voice Input, Speech ON).[^src-20260923-developmentguide-2]

All interface strings support full 4-language localization (English, Japanese, Traditional Chinese, Simplified Chinese) with 146 translation keys each.[^src-20260923-developmentguide-2]
...
requiring model calls. Relative reminders use this reference.[^src-20260923-developmentguide-2]
...
`/help <topic>` and `/guide 1..5` commands.[^src-20260923-ninjarobot-mcp-skill][^src-20260923-developmentguide-2]

On the web controller, movement failures surface as visible UI notifications in addition to
activity messages, preventing silent background task failures.[^src-20260923-developmentguide-2]
...
bounded manual sections (`project_documents.py`).[^src-20260923-developmentguide-2]

Phase 4 information-assistant tools and commands (`/info`) cover:
- **Calendar planning (T04)**: Google Calendar connect via port 8765 loopback SSH tunnel, primary-calendar auto-discovery, read-only by default, and explicit `--write` flag. Event creation, updating, and deletion (`calendar.events.preview_create/update/delete`) generate an exact 5-minute preview with field-specific validation errors (e.g. required timezone `Asia/Tokyo`, non-empty title, ISO timestamp range). Authorizing the exact preview requires a standalone single-word `CONFIRM` reply in chat; `CANCEL` discards it. Direct command bypasses are rejected. Updates and deletions are bounded to events created by the connection.[^src-20260923-developmentguide-2][^src-20260923-ninjarobot-mcp-skill]
- **Research service (T05)**: Allowlisted Tavily search with bounded snippets, three questions within 30 seconds, real source citations, and explicit save-to-note actions.[^src-20260923-ninjarobot-mcp-skill]
- **Notes, checklists, and briefings (T06)**: Local SQLite storage (migration 7), atomic note commit with consumed approval, on-request briefings using fixed local dates, and optional speech summary. Stale previews (>7 days) are cleaned on service startup; notes persist up to 1,000 records per kind.[^src-20260923-developmentguide-2]
