...
def test_normal_lint_warns_and_strict_lint_gates_stable_pages(project) -> None:
    ...
    page.write_text(
        valid_page("Semantic").replace("status: draft", "status: stable"), encoding="utf-8"
    )
    ...
    strict = [
        issue
        for issue in lint_project(config, strict=True)
        if issue.code == "semantic-review-missing"
    ]
    ...
    _add_review(page)
    assert not [
        issue for issue in lint_project(config, strict=True) if issue.code.startswith("semantic-")
    ]
    ...
    stale = [
        issue
        for issue in lint_project(config, strict=True)
        if issue.code == "semantic-review-stale"
    ]
...
def test_source_free_draft_page_does_not_require_semantic_review(project) -> None:
    ...
    assert not [
        issue
        for issue in lint_project(config, strict=True)
        if issue.path == "wiki/concepts/history.md" and issue.code.startswith("semantic-")
    ]
...
def test_absolute_language_stays_a_nonblocking_cue_on_source_free_drafts(project) -> None:
    ...
    page.write_text(
        valid_page("Absolute Draft", "This provisional rule always applies."), encoding="utf-8"
    )
    build_indexes(config.bundle_root)

    normal = [
        issue for issue in lint_project(config) if issue.path == "wiki/concepts/absolute-draft.md"
    ]
    strict = [
        issue
        for issue in lint_project(config, strict=True)
        if issue.path == "wiki/concepts/absolute-draft.md"
    ]
    assert any(
        issue.code == "absolute-claim-review" and issue.severity == "suggestion" for issue in normal
    )
    assert any(
        issue.code == "absolute-claim-review" and issue.severity == "suggestion" for issue in strict
    )
...
""",
    ...
    normal = [
        issue for issue in lint_project(config) if issue.path == "wiki/concepts/sourced-draft.md"
    ]
    strict = [
        issue
        for issue in lint_project(config, strict=True)
        if issue.path == "wiki/concepts/sourced-draft.md"
    ]
    assert any(
        issue.code == "semantic-review-missing" and issue.severity == "warning" for issue in normal
    )
    assert any(
        issue.code == "semantic-review-missing" and issue.severity == "error" for issue in strict
    )
...
""",
    ...
    strict = [
        issue
        for issue in lint_project(config, strict=True)
        if issue.path == "wiki/references/design.md"
    ]
    assert any(
        issue.code == "visual-review-missing" and issue.severity == "warning" for issue in normal
    )
    assert any(
        issue.code == "semantic-review-missing" and issue.severity == "warning" for issue in normal
    )
    assert any(
        issue.code == "visual-review-missing" and issue.severity == "error" for issue in strict
    )
    assert any(
        issue.code == "semantic-review-missing" and issue.severity == "error" for issue in strict
    )
