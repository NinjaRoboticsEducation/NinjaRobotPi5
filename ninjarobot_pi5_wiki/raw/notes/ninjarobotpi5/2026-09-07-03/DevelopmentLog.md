...
## 2026-09-07 — Refinement F02: emergency stop dispatch

Implemented the first foundation change from the approved refinement plan. Trusted
stop capabilities bypass ordinary queue capacity and resource locks, request
cancellation of competing work, and reject conflicting new work until stop dispatch
finishes. Cancelled lock acquisition releases already-held locks. The web controller
revokes permissions before dispatch and does not wait behind movement commands;
the assembly starts hardware cleanup before waiting for expression cancellation.
Repeated cancellation no longer interrupts an already-cancelling expression/idle task.

Changed IDE scheduler, execution engine, robot assembly and behavior cleanup, plus
Agent web control. Added five simulated regression tests. All 593 root tests passed
with the existing Starlette deprecation warning; lint, formatting, type checks,
compilation and both driver verifiers passed. Six managed drivers are unchanged.
No hardware, service deployment, microphone, camera or power action was run.

Full details, exact commands, limitations and the pending curated-wiki publication
are in the [F02 validation record](../../../../../docs/validation/refinement_f02_260907.md).
F01, F03, F04 and the later phases remain unfinished. This entry records this
feature only, not completion of the whole approved refinement plan.
