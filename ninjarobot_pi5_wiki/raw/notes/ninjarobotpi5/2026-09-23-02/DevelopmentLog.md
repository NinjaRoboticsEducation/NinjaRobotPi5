...
## Onboarding interface, reuse, and OAuth repair — 23 September 2026

Implemented the owner-approved refinement: ASCII welcome, numeric menus, one
bulk reuse option, automatic configuration checks and Enter-to-continue summaries.
Removed redundant YES/APPLY/media-consent phrases while preserving explicit
standalone capture choices and the raised-wheels servo precaution.

Added detailed Google Cloud and Mac SSH forwarding instructions, callback receipt
and waiting feedback, and an end-to-end read-only Calendar MCP validation call.
The observed Mac loopback redirect requires forwarding port 8765 to the Pi.
Credentials are staged before replacement; failed Calendar attempts clean up their
candidate file. Corrected invalid menu indexing, simulated-progress reuse, unsafe
retry recursion, and microphone asset-check import contamination. Manual commands,
managed drivers, authenticated web access, and the separate native Calendar remain.

Validation: 1,065 final full-suite tests passed in 45.58 seconds; all 457 driver
tests passed in separate processes. Ruff lint/format, mypy
(121 source files), compilation, CLI help/dry-run, driver baseline/source verifiers
passed. The final validation walkthrough records the latest full-suite result.
Known deprecation warnings concern Starlette/httpx and driver audioop. No live
Google account, Pi devices, services, boot configuration or power commands were
operated. A microphone import-containment regression found by the first full run
was repaired using an isolated read-only driver-helper subprocess.

Created new full manual versions and a beginner walkthrough once implementation
was complete. Existing registered source files remain unchanged. Wiki ingestion,
semantic review, map changes, and fingerprint refresh are intentionally deferred
by the owner. No dependencies/services were added; THIRD_PARTY_NOTICES.md needs
no change. Custom whisper.cpp paths still use the existing global launch flags.

## Curl installation repair — 23 September 2026

Resolved non-default branches through explicit remote references before detached
commit checkout; reject ambiguous branch/tag names. Confirmation now uses the
controlling terminal, with explicit failure without a terminal unless --yes is
set. Custom absolute destination support and the home-directory default remain
compatible. Real local Git and pseudo-terminal tests cover these paths: all 44
installer tests passed. No real installer, hardware, or system changes were run.
Final validation: 1,034 repository tests and 457 isolated driver tests passed.
Ruff lint/format, mypy (120 source files), compilation, shell syntax, document
links, both driver verifiers, and diff whitespace checks passed. ShellCheck is
not installed. Dependencies and external services did not change, so third-party
notices and the MCP manual need no update. Wiki ingestion and semantic review
remain deferred by owner instruction.


## Guided onboarding, curl bootstrap, and editable chat — 22 September 2026

The approved refinement adds a `ninjarobot` entry point and resumable onboarding
orchestrator around existing Agent, IDE, driver, provider, remote-access, and
configuration mechanisms. Required device setup remains in the managed drivers;
the new IDE adapter only owns locking and allowlisted process launch. No managed
driver source or immutable baseline changed.

Interactive terminal chat now uses prompt-toolkit. Arrow keys edit without
printing escape sequences, Enter sends, Shift+Enter or Alt/Option+Enter inserts
a line when the terminal distinguishes it, and Esc then Enter provides the SSH
fallback. The interface displays the key summary and `/help` hint.

The root installer retains its checkout behavior and can also bootstrap a new
checkout when streamed with curl. It supports revision and destination options,
safe dry-run/check behavior, private staging, required-file checks, exact SHA
verification when requested, existing-destination refusal, and cleanup. The
hardware installer adds an idempotent user launcher without starting the Agent
or operating hardware.

Onboarding covers five mandatory components, optional Bluetooth/microphone,
four model providers, external Tavily/Google Calendar/Notion MCP servers, ngrok
or authenticated same-Wi-Fi HTTPS, and simulation/real launch into existing
chat. Private state and credential stores use atomic writes, bounded schemas,
owner-only permissions, and symlink refusal. Notion OAuth is interactive only
during explicit setup. The Calendar MCP adapter is read-only and separate from
the existing native Calendar feature.

Tests cover terminal escape decoding and multiline editing, installer streaming
and safety, allowlisted IDE hardware dispatch, progress state, offline rehearsal,
MCP preset/auth/storage behavior, Calendar credential boundaries, CLI parsing,
wrong-mode service reuse, and web-access reporting. All hardware, external
account, and public endpoint acceptance remains operator-run. See the
[walkthrough and manual tests](../../../../../docs/validation/onboarding_install_chat_walkthrough_260922.md).

The final gate passed 1,021 repository tests and 457 separately invoked managed-
driver tests. Ruff lint and formatting passed for 463 files, mypy passed for 120
source files, compilation and both driver verifiers passed, and both Agent and
IDE wheel builds succeeded. Installer integration tests also found and repaired
an empty staging-directory leak before the final gate. ShellCheck was unavailable;
`bash -n` passed for both installer entry points. No automated run operated real
hardware, captured media, contacted an account, opened ngrok, or modified systemd.

At the owner's request, these source updates were prepared in one documentation
round without wiki ingestion or semantic review. The registered document map
still points to the prior checkpoint until a later publication workflow.
...
Software validation: 914 root tests passed; the same pre-existing public wiki
checkpoint test failed because its pinned hashes no longer match local pages.
Lint, format, compilation, mypy (112 source files), JavaScript syntax, and driver
verifiers pass. No dependency or service definition changed, so third-party
notices are unaffected. The existing real Agent was restarted to load the repair
and remains running. No user memory was cleared. Previous entries below are
historical; current evidence is in the linked walkthrough.


## Approved servo and distance recovery repair — 15 September 2026

The owner approved the follow-up audit plan. The earlier risk-label repair did
not address a stopped ramp being relabeled as a driver fault, or a cancelled
distance read leaving a healthy sensor unavailable. The IDE now preserves
intentional stop causes, routes normal stops through the motion owner, checks
obstacles before ramps as well as during movement, and provides consistent
confusing-face feedback. Actual driver/power/watchdog faults remain protected.

Distance preparation retains ownership of unfinished work, waits within a bound,
and checks health before automatic reuse. Game admission prepares the sensor
and reports unavailable devices. No manual enablement is required. A regression
test also verifies chat recovery after an oversized turn without deleting history.

Managed drivers, private configuration, saved safety data, provider dependencies,
and hardware interfaces were not changed. No live device tests were performed.
The Agent still accesses hardware only through the IDE. No third-party notices
change is needed because no dependency or service was added or changed.

For exact validation results, remaining baseline failure, manual acceptance, and
rollback instructions, see the [walkthrough](../../../../../docs/validation/servo_sensor_recovery_walkthrough_260915.md).
The final root suite reports 906 passed and one pre-existing project-help wiki
checkpoint failure. All 457 standalone driver tests pass. Lint, format,
compilation, type checks, both driver verifiers, and local document links pass.
Wiki registration, ingestion, review, and published fingerprint changes are
deferred at the owner's request. These successor sources are not published wiki
evidence. Earlier entries below describe their historical revisions.


## Servo stop, rolling chat context, and default games — 15 September 2026

### Root causes and changes

- `servo.stop` was labeled `EMERGENCY` to obtain priority engine dispatch even
  though the capability is routinely used by movement transitions, disarm,
  cancellation, and browser controller loss. The engine now assigns priority
  from its closed interrupt-capability allowlist, and `servo.stop` is correctly
  described as `LOW` risk. It still interrupts active pulses immediately.
  Explicit `behavior.stop` and unrecoverable driver failures retain Level 2
  Emergency Stop behavior and confirmed recovery.
- `AgentLoop` loaded the complete stored transcript into every provider request
  and rejected the request once its serialized form exceeded 128,000 characters.
  It now retains the full local transcript while selecting the largest recent
  suffix of complete user turns that fits each request. A trusted notice records
  omission, tool calls/results remain atomic, and a later short turn can recover
  after an oversized stored message. Required safety context is never removed.
- Refinement Phase 5 intentionally shipped the distance game behind a default-off
  configuration flag, which caused capability health and the bundled skill to tell
  users to enable it manually. The supported game is now always available subject
  to real module health, admission, bounded runtime, and system safety. The old
  `enabled` field remains accepted but has no gating effect, preserving private
  configuration compatibility.

### Scope, compatibility, and safety

The hardware path remains Agent to IDE to managed driver. No managed driver,
GPIO/I2C/SPI/PWM wiring, database schema, public request/result shape, service
deployment, privacy control, or power behavior changed. Automated validation does
not move actuators or prove Raspberry Pi hardware behavior. The supplied safety
snapshot had no fault or latch, Raspberry Pi throttling was `0x0`, and the later
Agent status command reported that the service was not running; therefore no live
service trace was available to attribute a physical event.

### Documentation and validation

This complete source version and matching Installation, Development, and MCP/Skill
manuals were created without modifying registered originals. At the owner's
request, they were not registered, ingested, normalized, prepared, or semantically
reviewed. The knowledge map and curated pages remain at their existing checkpoint.
Exact software-gate results and separate safe smoke, device-communication,
actuator-moving, and power-risk manual checks are recorded in the
[15 September validation record](../../../../../docs/validation/servo_chat_game_fixes_260915.md).
...
The owner resumed T04/T05/T06 after their earlier deferral. Implementation adds
reviewable local information and a narrow calendar connector rather than
changing generic external-tool trust or robot behavior. Interrupted work was
inspected and resumed; no existing user changes were discarded.

The final software checkpoint passed 887 tests, with the existing Starlette
TestClient deprecation warning. Lint, formatting, compilation and type checking
passed. All 222 managed-driver baseline files and the existing 56 authorized
repairs remain unchanged; all six drivers resolve to the workspace. Agent source
and wheel packages build successfully in an isolated build environment.
The handoff records the final consolidated gate and remaining manual acceptance.

Fake tests cover ownership, revisions, repeated approvals, restart, backup
restore, calendar conflicts/cancellation/lost responses, OAuth proof and state,
bounded evidence and exact note paging. Account authorization, real calendar
effects, speaker playback, deployment and wiki publication were not performed.

The earlier entries below are chronological history: their deferrals describe
the scope at that time, not the current delivery.

## Narrowed Phase 4 update — 13 September 2026

M03 memory ranking, M04 reviewed local read-only recipes, X01 version-2 Agent
skills and X05 public project help are implemented. Calendar work, new research,
notes, checklists and briefings remain deferred. Existing robot functions and
...
The first updated service start adds database revision 6 without removing old data.
...
Validation: focused recipe, skill, help and memory tests use synthetic data.
Final gate: 868 tests passed; lint, formatting (432 files), type checking
(103 sources), compilation, driver verification and wheel packaging passed.
263 local links and 58 anchors resolve. Details are in the linked handoff. The
retrieval evaluation measured zero misses across 50 queries over 1,002 synthetic
records. No physical acceptance or live configuration changes were performed.
The owner explicitly deferred all wiki ingestion and review workflows.
Preserve older log entries below as dated history.

## 2026-09-13 — Refinement Phase 5 software and hardware evaluation

Implemented optional silent conversational face variations and one bounded
hand-distance/buzzer/display game. Both default to disabled. Added strict run,
independent stop/status, terminal/chat/web controls and a version-1 bundled skill.
The Agent still authorizes typed tools; the IDE owns devices and coordination.
No wheel, capture or power action belongs to the game. Phase 4 remains deferred.

Fixed the shared sensor lifecycle so cancelled worker threads cannot overlap new
reads, health recovery or close. Unresolved work blocks reuse; explicit full health
recovery is required. Existing system-stop error contracts were preserved after
regression tests detected an initial admission error-mapping change. Added tests
for unavailable startup, failed silence and motion-only versus full recovery.

Four hardware option specifications and their index recommend retaining Bluetooth
and deferring additional hardware until missing evidence and approvals are in
place. No purchase, wiring, driver repair, live configuration or boot change occurred.

Validation details and actual counts are recorded in the
[Phase 5 handoff](../../../../../docs/validation/refinement_phase5_handoff_260913.md).
Fake-device tests, lint, types, driver integrity and browser simulation are distinct
from physical acceptance, which has not been performed for this implementation.
The [walkthrough](../../../../../docs/validation/refinement_phase5_walkthrough_260912.md)
contains manual tests and rollback. Registered originals and curated wiki pages
remain untouched. Wiki ingestion/review is deferred by the owner's instruction.
...
Validation: 796 main-suite tests passed, including six new clock-focused tests.
Lint, formatting, type checks, compilation, driver verifiers and documentation
links passed. The Pi clock was read successfully with Asia/Tokyo (+09:00).
No live Agent restart or scheduled reminder was executed during validation.
...
Validation: 86 focused game/Agent-loop tests passed; the full suite had 925
passes and the previously recorded public-wiki-checkpoint failure. All 71
distance-driver tests passed. Ruff lint/format, mypy (112 files), compilation,
JavaScript syntax, immutable-driver and workspace-source checks passed.
Live acceptance of this patch remains pending. The owner confirmed waiting for
the final chat answer before presenting their hand, which explains the old
two-second no-reading exit. No private data was cleared and no driver changed.
...
## 17 September 2026 — Calendar setup and current project help

Validation: 944 root tests passed in 41.34 seconds, including the formerly failing
public-checkpoint test. Ruff lint/format (451 files), mypy (113 files), compilation,
managed-driver integrity and workspace-source checks passed. The Agent package
source archive and wheel built with the new retrieval module and manifest.
All 197 checked local documentation links resolve. One existing Starlette
deprecation warning remains. These results do not claim real Google authorization
or event acceptance; those require the owner's manual test.

Approved implementation: fixed the missing reviewed read declaration, added
primary-calendar discovery and automatic user-owned registration to protected
OAuth setup, and made reauthorization reuse the connection ID. Event writes
retain exact preview/confirmation. Added a compatibility authorization wrapper.

Replaced default historical four-page retrieval with hash-checked current public
manual sections and bounded continuation IDs. Runtime evidence includes explicitly
listed unpublished sources without wiki ingestion. Retained private-path and
symlink rejection. Added tests for setup, discovery, permissions, reconnects,
current-source retrieval, continuation, and confinement.

The Agent remains stopped. No Google consent, event write, capture or motion was
performed. Local MCP discovery now exposes the expected read tool. New Google
write consent and live user acceptance remain pending. See the
[walkthrough](../../../../../docs/validation/calendar_wiki_walkthrough_260917.md)
for test results and exact manual steps.
...
### CONFIRM follow-up validation

956 root tests passed in 41.99 seconds, with one existing Starlette deprecation
warning. Eighteen focused information/chat tests passed. Ruff lint/format
(452 files), mypy (114 source files), compilation and driver/workspace-source
checks passed. Tests cover missing timezone correction, exact preview delivery,
plain CONFIRM, cancellation, expiry, user/session isolation, changed account,
failed delivery, supersession, double confirmation and uncertain writes.
No Google write or service restart was performed. Manual acceptance remains
pending; documentation was updated once without wiki ingestion or review.
...
## 2026-09-17 — One confirmation flow for all Calendar event changes

The owner approved extending plain CONFIRM from creation to editing and deletion,
and rejecting the old direct confirmation route. The creation-only guard and
prompt instructions caused inconsistent guidance. Deletion also lacked readable
saved event details. CalendarService now saves bounded existing details with the
preview; CalendarChat renders the target and proposed changes and alone supplies
the internal confirmation authority to the controller. The model cannot supply
that authority through arguments. A Google version conflict returns rejected and
requires a fresh preview, while unknown outcomes remain uncertain without retries.

Validation: 54 focused information tests passed, including all three operations,
cancellation, expiry, user/account/session changes, failed delivery, duplicate
confirmation, command bypass rejection, and update/delete version conflicts.
See the [final walkthrough validation record](../../../../../docs/validation/calendar_confirm_walkthrough_260917.md)
for full gates. No live Google writes, Agent restart, or hardware actions were run.
The four successor source manuals and runtime public-help hashes were updated;
wiki ingestion and review remain for the owner after manual tests.


## 2026-09-18 — Web interface refinement and terminal menu reorganization

### Objective and architectural context
Implemented the user interface refinement plan to provide dedicated operational surfaces for game pad driving and AI agent conversation, modularize frontend assets, allow safe execution of user-created behaviors from the web UI, standardize button styling and toggle behaviors, and streamline the terminal IDE interactive menu.

### Summary of implementation
1. **Dual Web Surfaces**: Split web interface into `agent.html` (`/`, `/agent`) and `gamepad.html` (`/gamepad`). Hamburger menu organizes items into Language, Control Interface, and System Power.
2. **Frontend Modularization**: Replaced monolithic `app.js` with `app-shared.js`, `app-gamepad.js`, and `app-agent.js`. Shared state handles lease renewals, i18n, and websocket transport.
3. **User Behaviors in Web UI**: Integrated `list_user_behaviors` and `run_user_behavior` into `web_control.py` and `web_app.py`, hooked to IDE `_BehaviorListAdapter` `source="user"`. Motion confirmation modal protects against accidental actuator activation.
4. **Button Styling and State Toggles**:
   - Special robot controls (A/B/X/Y on Agent view): Black text / black border when disabled; white text / white border when enabled.
   - Audio controls (Web Mic, Voice Input, Speech ON): White text / white translucent border when disabled; blue text / blue translucent border when enabled.
5. **Terminal IDE Restructuring**: Replaced cluttered top-level menu with streamlined options 1–5 and Q:
   1. Hardware Configurations, 2. Simulation, 3. Run Robot Behaviors, 4. User-created Behaviors, 5. Bluetooth Speaker Connection, Q. Quit.
   Removed single-key 'e' emergency stop injection from menu prompts.
6. **Localization**: Added 12 new translation keys across English, Japanese, Traditional Chinese, and Simplified Chinese (146 keys each).

### Validation and test gates
- Bytecode compilation: Clean across IDE and Agent packages.
- Immutable driver baseline: 222 tracked files matched baseline and authorized repairs.
- Static analysis: Ruff format and lint clean; Mypy passed across all source modules without errors.
- Automated test suite: 125 unit tests passed, including mobile interface assertions, user behavior list/run tests, and button styling initial state tests.
- Safety: Automated tests did not actuate physical hardware or modify system services. Manual Raspberry Pi validation checklist prepared in validation walkthrough.
