...
### Optional game setup and operation

Add `[distance_game]` with `enabled = true` and `volume = 16` to the existing robot
configuration while the service is stopped. Volume accepts 1–32. Do not duplicate
an existing TOML section. Restart using the existing service manager and account;
never start a second hardware owner. The full walkthrough explains both system
service and CLI restart methods. No new runtime dependency or boot service is needed.
