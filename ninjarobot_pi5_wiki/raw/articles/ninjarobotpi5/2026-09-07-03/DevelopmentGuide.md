...
### Stop dispatch and cancellation — F02 refinement

The IDE gives its three trusted, argument-free stop capabilities — `behavior.stop`,
`servo.stop` and `buzzer.stop` — an interrupt path (a route that bypasses ordinary
queued work). Their existing names, arguments, result records and duplicate-request
handling are preserved. Only these registered emergency capabilities use this path;
ordinary actions cannot request priority through model arguments.

A stop requests cancellation of queued and running actions sharing its resources,
without waiting for those actions to finish cleanup. New conflicting work receives
`ACTION_STOP_IN_PROGRESS` while a stop is executing. Independent resources remain
available, and concurrent stop requests do not cancel one another. Cancellation
releases any resource locks already acquired while waiting for other locks.

The browser revokes direct-control, chat and voice motion permissions before stop
execution, and dispatches Emergency Stop without waiting behind an earlier movement
command. The assembly starts device stop alongside expression/idle cleanup so a
blocked expression does not delay dispatch to the motors. Existing stop deadlines
and error reporting remain; queue independence does not prove a maximum physical
stopping time or make an unresponsive driver cancellable.

The separate F01 raw-servo safety, F03 legacy ownership and F04 disabled-device/pin
repairs remain planned. This F02 change does not certify those paths or enable new
robot autonomy. See the [F02 validation record](../../../../../docs/validation/refinement_f02_260907.md)
for simulated checks and the separate manual Pi checklist.
...
See the [MCP and Agent Skills Tutorial](../2026-09-07/NinjaRobot_MCP_Skill.md) for the
...
See the [MCP and Agent Skills Tutorial](../2026-09-07/NinjaRobot_MCP_Skill.md) for the exact
