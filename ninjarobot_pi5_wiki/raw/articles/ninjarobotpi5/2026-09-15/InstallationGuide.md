...
A cancelled or timed-out sensor worker remains owned until it returns. Explicit
full system resume performs recovery health checks; movement-only resume does not
clear a game fault. A permanently stuck worker requires operator/service recovery;
do not bypass checks or start another process against the device.
