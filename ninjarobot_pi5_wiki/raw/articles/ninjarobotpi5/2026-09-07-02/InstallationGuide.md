...
### Emergency Stop during busy work — F02 refinement

Emergency Stop uses a dedicated IDE dispatch path even when ordinary work fills the
queue. The web controller revokes motion permissions before sending it. An action
submitted while the same devices are being stopped may report `ACTION_STOP_IN_PROGRESS`;
wait for stop/recovery feedback and issue a new command deliberately. Interrupted
work is not automatically resumed by this refinement.

Dispatching a stop is different from proving that every physical device stopped.
A blocked driver or failed cleanup can still produce a timeout or error. Keep the
existing raised-wheel precautions and an operator ready to remove power for manual
movement checks. The [F02 validation checklist](../../../../../docs/validation/refinement_f02_260907.md)
separates software checks from unperformed Pi tests. Other foundation repairs remain
planned; this change is not an approval for expanded autonomous movement.
