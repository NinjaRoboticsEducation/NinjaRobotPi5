...
## Onboarding refinement and audit — 23 September 2026

The wizard remains an Agent orchestration layer over IDE contracts. The bulk
reuse menu calls `hardware_setup_status` for each module; IDE discovery shares
`discover_pi5_configs`, including XDG configuration paths and existing supported
project-local fallback files. Bounded JSON checks and `import_pi5_configs` reuse
the integrated schema. Servo checks reuse its calibration manager and require
saved neutral points for both wheel pins. Configuration checks never move devices.

Microphone asset checks reuse standalone path resolvers through the bounded
`ninjarobot_pi5_ide.hardware_config_probe` subprocess. Do not import those driver
modules directly into the IDE process: doing so violates its deliberate
microphone runtime import containment. The probe does not record, transcribe,
load an inference model, or install software. Driver sources remain unchanged.

The wizard applies one consolidated hardware import, preserves unrelated robot
configuration, validates numeric selections, and uses iterative recovery. Resume
rejects simulated records, rechecks settings, and records configuration validity
separately from physical tests. Offline rehearsal leaves existing progress intact.

`calendar_oauth.authorize` accepts an optional output callback while preserving
its previous call signature. It uses loopback-only binding, state and PKCE checks,
a five-minute browser deadline, periodic waiting messages, explicit receipt, and
listener cleanup. Calendar onboarding explains the Mac SSH forwarding boundary
before generating a link. MCP validation launches the existing external server
and checks a bounded read-only events response after reviewed-tool discovery.

Candidate API keys stay in memory until validation; existing SecretStore name,
value and private-file validation is preserved. Configuration-save failures roll
back changed file-backed keys. Calendar reconnect uses a unique mode-0600 file,
removing a failed candidate rather than overwriting working credentials. OAuth
setup has a separate bounded interactive allowance; ordinary service MCP timeouts
remain unchanged and cannot trigger browser authorization.

Integration coverage includes a real local Calendar MCP subprocess with its Google
transport mocked, callback success/denial and existing state/PKCE tests, module
reuse and missing-module handoff, bad input, secret rollback, simulation isolation,
and microphone runtime containment. Tests never perform live account or device actions.

See [validation evidence and manual tests](../../../../../docs/validation/onboarding_refinement_walkthrough_260923.md). Wiki ingestion and semantic
review are deferred to the owner after Raspberry Pi testing. No dependency or
service was added; third-party notices are unchanged.
...
## Guided setup architecture — 22 September 2026

`ninjarobot` and `ninjarobot-agent` share the existing argparse dispatcher.
`setup_wizard.py` is a presentation and orchestration layer: it owns progress,
selections, concise explanations, retry/skip decisions, and final launch. It does
not access a managed `pi5*` package. Hardware setup follows the permitted path:
...
The IDE adapter resolves a fixed command, acquires `HardwareOwnership`, attaches
the existing interactive tool to the terminal, and releases ownership in a
`finally` block. Managed driver behavior and immutable sources are unchanged.
Configuration import reuses `discover_pi5_configs`, `import_pi5_configs`, and
`save_robot_config` once after the selected hardware steps.

Progress uses a versioned JSON object at
`~/.local/state/ninjarobot_pi5/setup-progress.json`. The store uses a nonblocking
owner lock, bounded input, schema validation, symlink refusal, mode `0600`, and
an fsynced atomic replacement. It contains no secrets or media. `--dry-run` and
`--status` do not create state; simulated verification cannot satisfy a later
real-hardware readiness, even when component configuration exists.

Provider setup reuses `SecretStore`, `ConfiguredProviderRegistry`, dynamic model
catalogs, provider health, benchmark evidence, and `persist_model_selection`.
MCP setup preserves custom servers and saves a reviewed preset only after MCP
initialization and allowlisted tool discovery. Tavily uses bearer authentication;
Notion uses the SDK OAuth client with private token storage; Google Calendar is
a separately launched stdio MCP process that reuses the existing bounded Google
OAuth and HTTP code. Runtime service startup cannot initiate interactive Notion
authorization; the operator must use the onboarding MCP step.

The final launch reuses `_spawn_service` and `_chat_repl`. Service status exposes
`execution_mode`, so a running simulation instance cannot be mistaken for the
requested real-hardware mode. After startup the CLI reports verified ngrok state
or authenticated local HTTPS readiness. Exit does not start a service.

`terminal_input.py` uses prompt-toolkit only for interactive terminals and keeps
plain input for redirected/test streams. Its bindings submit on Enter, insert a
newline for recognized Shift+Enter sequences or Meta/Option+Enter, and leave
ordinary arrow navigation to prompt-toolkit. Esc then Enter is the portable
modifier fallback.

The root `install.sh` retains adjacent-checkout delegation and adds a standalone
bootstrap mode for streamed downloads. It stages a new clone under the requested
parent, checks a supplied exact revision when it is a 40-character commit,
verifies required files, then moves the checkout and invokes the unchanged
installer interface. The hardware installer adds an idempotent
`~/.local/bin/ninjarobot` symlink and refuses to replace an unrelated launcher.

Automated tests fake hardware, OAuth, MCP, providers, services, and terminal
input. They never move motors, capture media, alter systemd, or open a live
tunnel. Raspberry Pi and account acceptance remains manual; use the
[walkthrough](../../../../../docs/validation/onboarding_install_chat_walkthrough_260922.md).

## Google Calendar authorization and account setup — 17 September 2026

The supported setup now connects your primary Google Calendar to the Agent after
one browser authorization. You do not need to copy a calendar ID or run a second
account-registration command. Reading schedules and creating reviewed events use
the existing built-in Calendar tools. Google consent is still required; an old
read-only token cannot grant event creation permission.

### Before Google Calendar authorization

1. In Google Cloud, enable the Google Calendar API (the service used to read and
   write events). Configure the consent screen and add your Google account as a
   test user if the application is in testing mode. Create a **Desktop app**
   OAuth client (the credentials used for browser sign-in).
2. Save the downloaded JSON as
   `$HOME/.config/ninjarobot_pi5/google-calendar/credentials.json` on the Pi. Keep
   the directory private and the file readable only by your account:
   ...
   If the file is missing, copy your downloaded Desktop client JSON there first.
   Never paste its contents or authorization tokens into robot chat.
3. Start your normally configured Agent. For a manually managed installation,
   run from the project checkout on the Pi:
   ...
   This starts the real robot service. Do not start a second instance if a system
   service already owns the robot. A stopped Agent produces a clear setup error;
   the authorization command does not silently start it.

### Authorize Google Calendar from Raspberry Pi OS Lite

1. On your laptop/desktop computer, open an SSH tunnel to the Pi. Replace the
   example login and host with your Pi's actual address:

   ```bash
   ssh -L 8765:127.0.0.1:8765 YOUR_PI_USER@YOUR_PI_HOST
   ```

   Keep this terminal connected. The browser runs on your computer; the Pi does
   not need a desktop. Do not open or forward this port on your router.
2. In that SSH session, run:
   ...
   The default credentials path is used automatically. Omit `--write` for a
   read-only connection. Optional `--client-file`, `--calendar-id`,
   `--account-label`, `--port`, and `--session` overrides remain supported.
3. Open the printed Google URL in your computer's browser. Choose the intended
   account and approve the requested event permission and calendar-list read
   permission used to identify your primary calendar. Complete the flow within
   five minutes. The browser should say authorization was received; return to
   the Pi terminal. Do not paste the callback URL into chat.
4. The command discovers the primary calendar, verifies its identity/permission,
   and registers the connection for the local robot user. A successful result
   includes `connection_id`, `calendar_id`, and `write_enabled`. Reauthorizing the
   same calendar updates its existing connection instead of adding a duplicate.
   No additional Agent restart is needed after registration.
5. In terminal or web chat, run `/info calendar.connections`, then ask “What is
   on my calendar today?” Use the same local robot user profile that performed
   setup; another user's private calendar is intentionally not visible.

The compatibility command for the existing example installation is:
...
On this repaired installation it delegates to the supported setup above and
requests read plus reviewed event creation. Use `--read-only` for read-only setup.
The legacy external MCP reader retains its separate existing `token.json`;
this command establishes the built-in connection and does not overwrite that
legacy token or change its account. Prefer the built-in connection for normal
schedule questions and all reviewed event creation. New installations can use
`python scripts/authorize_google_calendar.py` from the robot checkout instead.

### Review and confirm a Google Calendar change

Ask the Agent to prepare a new event with a title, date, start/end times, and time
zone. It should show a preview naming the account and calendar. Nothing has been
created at this point. Confirm only after checking those details.

After the exact runtime preview appears, reply in the same chat:

```text
CONFIRM
```

Reply `CANCEL` to discard it. No operation ID or review hash needs to be copied.
The runtime binds CONFIRM to the delivered preview and active local user. A new
preview replaces the previous confirmation; other messages discard it. Failed
preview delivery, service restart, expiry, changed account, or another user/session
cannot authorize a write. Duplicate CONFIRM does not create another event.
Direct `/info calendar.confirm` and controller confirmation requests are rejected;
only a standalone CONFIRM reply to a delivered preview can authorize a change.

This confirmation can create a real event. Previews expire after five minutes;
request a fresh preview if expired. Check the reported result and Google Calendar.
If the result is uncertain, inspect/reconcile the saved operation instead of
requesting a duplicate creation. Existing limits remain: no attendee invitations
or recurring-event changes, and no unrestricted external MCP writes.

### Google Calendar tool missing or disconnected

For the legacy example MCP reader, both local declarations are required:
...
Add them only to the inspected `google-calendar` server section. The second line
records that this tool was reviewed as read-only; it must never name a write tool.
Reload the Agent after changing MCP configuration. To inspect discovery without
reading calendar events, run:

```bash
uv run --frozen --no-sync ninjarobot-agent mcp tools google-calendar
```

Expect `mcp.google-calendar.list_today_events`. A connected server exposing zero
tools now reports degraded status and explains the missing review declaration.
An empty built-in `/info calendar.connections` list does not by itself prove an
external MCP reader is unavailable. Expired/revoked Google grants require a new
browser authorization; genuine permission failures are not bypassed.

## Current project wiki retrieval from robot chat

Ask a question such as “How do I authorize Google Calendar on Raspberry Pi OS
Lite?” or “Explain the hardware architecture.” No wiki engine setup or ingestion
command is required in chat. `/project Google Calendar authorization` also returns
cited evidence directly. Terminal and web chat use the same read-only provider.

The reader searches complete public manuals resolved from the current document
map. This release also explicitly names the new unpublished manuals in
`project_help_sources.json`, so current instructions are available before wiki
ingestion. Results clearly label those sources as unpublished. A newer registered
source supersedes the release manifest automatically, including same-day versions.

Search returns up to three relevant sections. `project_help.read` reads a returned
`document_id`; `next_document_id` continues long procedures. Sections are limited
to 3,000 characters, with source paths, headings, dates, and content hashes
(digital signatures that detect changes). The Agent should follow continuation
IDs rather than claim an instruction is missing after reading one short excerpt.

Only the four named public manuals are admitted through this current-source path.
The older explicit-manifest reader remains available for compatible callers.
Private files, traversal outside the wiki, symbolic links, oversized files, and
hash-mismatched sources are rejected. No retrieved command is executed. A missing
checkout or stale source produces a warning, not a fabricated answer. This reads
source manuals directly; it does not invoke the wiki engine or certify wiki pages
as reviewed. Developers must deliberately update the runtime source manifest when
adding unpublished manual revisions; never silently refresh hashes to hide drift.


## Web controller and repeated-game follow-up — 15 September 2026

The live web failure was an action-admission rejection, not an Emergency Stop:
a game-related check blocked movement before automatic sensor recovery could run.
That redundant check is removed. The motion controller still owns bounded sensor
recovery and all motor safety checks. Web movement failures now appear as visible
notifications as well as activity messages, instead of disappearing in a background
task. Normal release and obstacle recovery do not require Resume.

Game cleanup deliberately releases the buzzer backend. Each new game now silently
initializes it before health checks; a stopped buzzer is not treated as a permanent
fault. An initialization failure still blocks the game. New explicit game requests
use fresh tool results rather than treating an earlier failure in chat as permanent.
The game never restarts itself after failure, cancellation, or a safety stop.

The motion watchdog heartbeat now continues through responsive device cleanup.
Slow cleanup alone no longer looks like a frozen control loop. Actual control-loop
stalls still trigger protective stopping; no managed driver or hardware interface
was changed. Existing memory and private configuration were preserved.

The owner confirmed successful web movement and obstacle-clearance tests without
Resume. Three live five-second games, with Stop commands between runs, completed
with valid sensor readings; the owner confirmed sound and display on every run.
See the [walkthrough and validation record](../../../../../docs/validation/web_game_followup_walkthrough_260915.md)
for evidence and repeatable manual checks. Wiki ingestion and review remain deferred.


## Approved servo and sensor recovery follow-up — 15 September 2026

This successor documents the approved follow-up repair. Wiki ingestion and
semantic review (checking published claims against their evidence) are deferred.
The registered document map still refers to the earlier published checkpoint.

- Normal stops and detected obstacles do not require Resume. Three consecutive
  fresh readings at or below the configured threshold (default 50 mm) stop or
  prevent a servo movement, including backward and direct endpoint commands.
  A confusing face and a clear message explain the temporary interruption.
- After clearing the obstacle, issue a new command. The old movement never
  restarts itself. An already detected obstacle is not treated as cleared by a
  failed or stale reading. The front sensor cannot protect the rear or sides.
- Explicit Emergency Stop still requires deliberate recovery. Genuine power,
  driver, and control-loop faults retain protective stopping and report the
  fault; ordinary driver aborts caused by a requested stop are not new faults.
- A cancelled distance read retains exclusive device ownership until its worker
  finishes. The next reading or game request can recover through a bounded wait
  and health check. A stuck worker or failed check remains unavailable. Games
  need no manual enablement and never command the wheels.
- Long-chat context trimming remains in place. A short turn can recover after an
  oversized turn, without erasing history or replaying a tool action.

See the [repair walkthrough and validation record](../../../../../docs/validation/servo_sensor_recovery_walkthrough_260915.md).
No physical acceptance is claimed by these software changes.


## Phase 4 information assistant — 13 September 2026

T04 calendar planning, T05 research with evidence and T06 notes/checklists/
requested briefings are now implemented alongside M03, M04, X01 and X05.
Existing robot functions and optional Phase 5 interactions are preserved.

Start with the [complete information-assistant walkthrough](../../../../../docs/validation/refinement_phase4_information_walkthrough_260913.md)
for exact Raspberry Pi OS Lite terminal commands, Google setup through an SSH
tunnel, local notes, review/confirmation, research, briefings and recovery.
The [handoff](../../../../../docs/validation/refinement_phase4_information_handoff_260913.md)
records implementation details, validation and remaining manual account/speaker
acceptance. No live account or hardware acceptance is claimed.

The new information CLI and /info chat commands use the existing service and
private database. Read/list commands and local-only briefings need no conversation
model. Every note edit and calendar write has a five-minute exact preview and a
separate same-session direct confirmation. Models cannot confirm changes.

Google Calendar remains disconnected until explicit calendar-connect setup.
Read-only is the default. A separate --write authorization creates a new connection.
Credentials use the existing owner-only secret store. Calendar events never
silently create or change local reminders. Updates preserve unspecified Google
fields and require the previewed remote version. Uncertain writes are reconciled
by reading their saved event IDs, never automatically repeated.

Research uses the existing allowlisted Tavily server and provider snippets.
Three search questions fit within 30 seconds and the current request budget.
Real source IDs, links, dates and qualified interpretations make evidence
reviewable; citations alone do not prove truth. Saving a result is a separate
reviewed note action. Usage limits are not a monetary spending cap.

Briefings run only on request, use a fixed local date and selected sources,
retain useful local text if an optional provider fails, and give a short summary
through existing speech when enabled. They do not save themselves or run at boot.

Migration 7 adds owned information records to the existing database. Notes,
checklists and calendar receipts are covered by the existing private backup.
Restored unused approvals expire on startup; a backup cannot undo remote changes.
Research runs and note previews older than seven days are cleaned on service
startup. Saved notes and calendar receipts persist until explicit deletion or
profile reset, with a 1,000-record limit per user per kind.

Wiki ingestion, review and publication were explicitly skipped. This is a new
complete source version; registered originals, published pages and the knowledge
map are unchanged. Runtime project help labels registered versus unpublished source evidence.

### Implementation map

InformationStore owns short user-scoped SQLite transactions. NotesService commits
the note and consumed approval together. CalendarService manages exact approvals,
remote verification and uncertain outcomes; GoogleCalendarBackend owns bounded
fixed-host transport and credential refresh. The calendar_oauth helper handles
the separately invoked Desktop browser flow and loopback cleanup.

InformationProvider exposes read/proposal tools and InformationControls handles
direct confirmation through existing PolicyEngine and TaskService receipts.
AgentRuntime, IPC (local inter-process messages), CLI and web chat reuse these
controls. Existing recipes retain their narrow read-tool allowlist.

ResearchService binds evidence to a saved run and offers qualified, dated
reference rendering. BriefingService creates bounded evidence bundles and
deterministic text. AgentLoop carries an optional short speech summary internally;
public response serialization and existing hardware interfaces are preserved.

The additions live under ninjarobot_pi5_agent/src/ninjarobot_pi5_agent.
No Pi5 library, robot hardware contract, operating-system setting or boot service
was changed. The existing HTTPX dependency handles Google transport; no new
Google SDK (software development kit) is installed.

## Earlier Phase 4 foundations — retained functionality

M03 memory ranking, M04 reviewed local read-only recipes, X01 version-2 Agent
skills and X05 public project help are implemented. T04/T05/T06 information
features are delivered in the newer section above. Existing robot functions and
Phase 5 optional interactions are preserved.

Follow the [Phase 4 walkthrough](../../../../../docs/validation/refinement_phase4_walkthrough_260913.md)
for command-line setup, expected results, recipe preview/save/run/versioning,
memory checks, skill compatibility and rollback. The
[handoff](../../../../../docs/validation/refinement_phase4_handoff_260913.md)
records boundaries, validation and remaining manual acceptance.

The Agent now supports /project QUESTION and /recipes list, show, run and disable.
The recipe CLI adds explicit preview/save, inputs, enable/delete and rollback.
Saved recipes cannot move hardware or schedule reminders. Each step uses the
existing policy and task receipts; saved versions are not permission to run.
Database revision 6 remains supported; the current service adds revision 7
without removing old data.

Public project help reads bounded sections of the current full public manuals.
Registered and explicitly listed unpublished sources retain separate labels.
See the 17 September setup and retrieval instructions at the start of this manual. Version-1 skills remain supported; version 2 adds
requirements and fallbacks with no extra permissions. No new dependency, operating
system setting, boot deployment or driver change is required.

Implementation lives in the Agent's memory_store.py, memory_services.py,
recipes.py, recipe_controls.py, skills.py and project_help.py. Migration 6 adds
owned recipe heads and immutable numbered versions to the existing SQLite
database (a local file-based data store). Existing request receipts handle
cancellation and restart; interrupted work is never replayed automatically.
Closed read-tool selection and per-step policy preserve the hardware boundary.

Use the [skill templates](../../../../../docs/skill-templates/README.md) and
[synthetic retrieval check](../../../../../scripts/validate_phase4_memory.py).
A recipe is an owned workflow; a runtime skill is an installed instruction
package; developer wiki skills remain a separate development workflow.
Hardware interfaces, managed drivers and the legacy v1 skill contract are intact.

## Current refinement checkpoint — Phase 5, updated 15 September 2026
...
The supported distance game is available by default; expression variations remain
disabled by default. The subsequent narrowed Phase 4 delivery is described above. Physical Phase 5
...
Priority interrupt dispatch is identified by the engine's closed stop-capability
allowlist, not by an `EMERGENCY` risk label. `servo.stop` is therefore a low-risk,
empty-argument normal stop that still bypasses the ordinary worker queue and
interrupts pulses immediately. The distinct `behavior.stop` operation remains the
explicit Level 2 Emergency Stop path. This preserves stop latency without exposing
normal movement transitions as system emergencies in tool discovery or status.
...
- Trigger: three consecutive valid front readings at or below 50 mm during
  any integrated servo movement, including backward movement and direct endpoints
- Result: stop the servos, cancel the behavior's concurrent and later stages,
  show a bounded silent confusing face, report the cause and next step, then return
  to Idle
...
- Triggers: Raspberry Pi undervoltage, software watchdog timeout, or a servo
  unexplained driver interruption (not an ordinary requested stop)
- Recovery: `ninjarobot-ide-tool motion resume --confirm`

**Level 2 (system stop)** — stops servos and ranging, suspends camera and microphone backends, and silences the buzzer. Explicit operator stops display the Emergency Stop sign; hardware faults display Protective Stop. Confirmed recovery performs health checks and reconstructs/restarts modules as needed.
...
`servo.stop` is also an ordinary priority interrupt, not a Level 2 trigger. It is
used for movement transitions, disarm, cancellation, and controller loss. Its
adapter risk is `LOW`; the engine's closed interrupt allowlist supplies immediate
dispatch. `behavior.stop` and unrecoverable hardware failures retain Level 2
semantics and confirmed recovery.
...
Conversation storage and model input are deliberately separate. The SQLite
transcript retains all messages allowed by the configured retention policy. For
each provider call, `AgentLoop` measures the complete serialized `ModelRequest`.
When it exceeds `max_prompt_characters`, the loop chooses the largest suffix of
complete user-boundary turns that fits, adds a trusted omission notice, and leaves
the stored transcript unchanged. Selection uses a logarithmic suffix search, and
tool-history repair keeps assistant tool calls with their results. A current turn
that cannot fit with immutable safety, runtime, memory, skill, and tool context is
rejected explicitly instead of dropping required trusted context. The mechanism is
provider-independent; the configured ceiling must still be appropriate for the
...
- Before a ramp, check for a nearby obstacle; monitor throughout movement.
- The VL53L0X raw sentinel `8191` reports no measurable target. It is not proof
  that a previously detected obstacle has cleared.
- Invalid or stale readings retain the existing warning behavior unless an
  obstacle was already detected; then movement remains temporarily blocked
  until a fresh reading confirms clearance.
- Three consecutive valid readings ≤ the configured threshold (default 50 mm)
  stop or prevent every integrated servo movement without a persistent latch.
...
- A silent confusing face appears for two seconds, then supervised Idle resumes
...
- Backward movement also obeys the front obstacle stop; rear hazards remain
  outside the sensor view. Legacy `warn_only` input is accepted for compatibility
  but cannot bypass this obstacle guard.
...
## Distance game hand-placement correction — 16 September 2026

The game starts before the final chat answer. Place your hand 5–60 cm in front
of the sensor when asking to play. Web chat now shows a starting instruction
before the game finishes. Do not wait for the final answer to begin playing.

A fresh reading outside the playing area, or the sensor reporting no target,
keeps the game silent until a hand arrives or the requested timer ends. It no
longer stops after two seconds and incorrectly reports that the sensor is
unavailable. A game with no hand detected finishes with `no_target_detected`.
Real read errors and stale readings still stop after two seconds without a
healthy reading. No safety checks, device drivers, wheel controls, or permissions
were relaxed.

The result retains existing counters and adds `no_target_samples`,
`rejection_counts` (counts by rejection cause), and `last_sample_issue`.
`invalid_samples` still counts all samples rejected for tone generation,
including no-target samples; it is not a count of hardware failures.
The Agent Skill distinguishes no target from a device fault and explains the
returned result instead of treating historical failures as permanent.

See the [repair report and step-by-step tests](../../../../../docs/validation/distance_game_web_hand_260916.md).
These successor manuals are not ingested or semantically reviewed; the published
wiki document map remains unchanged at the owner's request.


## Calendar preview validation and CONFIRM — 17 September follow-up

Missing tool arguments are reported as local validation errors, not “Source
unavailable.” An event requires title, start, end, and timezone; the model must
carry a supplied zone such as Asia/Tokyo into `event.timezone` even when timestamp
offsets are present. Relative dates use the system date. Preparing a create
preview makes no Google write request. Write-enabled connection state alone is
not proof that a later Google write will succeed.

The runtime displays the stored account, calendar, action, title, start/end,
timezone, location, description and expiry. Reply CONFIRM to authorize that exact
preview; CANCEL discards it. Confirmation is processed directly, without model
inference. The saved operation/hash, ownership, connection revision, expiry and
single-dispatch protections remain internal and authoritative. If Google returns
an uncertain result, inspect the saved operation instead of creating a duplicate.

See the [CONFIRM walkthrough and manual tests](../../../../../docs/validation/calendar_confirm_walkthrough_260917.md).
Wiki ingestion and review remain deferred until the owner's manual validation.
...
## Unified Calendar confirmation: 17 September follow-up

Creation, editing, and deletion use the same preview and CONFIRM flow. An edit
shows existing and proposed details. A deletion shows the existing title, dates,
calendar, and event ID and explicitly says DELETE event. CANCEL only discards
that preview; it does not delete an event. Read-only schedule questions need no
confirmation. A bare CONFIRM is accepted regardless of letter case and surrounding
spaces; sentences such as "yes, confirm it" do not authorize changes.

A changed event version is rejected by Google without applying this change; ask
for a fresh preview. A timeout or uncertain result must be checked before retrying.
Existing expiry, account/user/session binding, ownership, and duplicate protection
remain. No new dependency, Google permission, or hardware change is required.
Old pending previews should be requested again after restarting the Agent.

Follow the [create, edit, and delete manual tests](../../../../../docs/validation/calendar_confirm_walkthrough_260917.md).


## Web interface refinement and modular frontend architecture: 18 September 2026

### Modular frontend architecture
The monolithic `app.js` and `index.html` have been refactored into a modular frontend:
- `agent.html`: Dedicated structure for the Agent Interface with expanded conversation viewport, chat composer, and categorized controls.
- `gamepad.html`: Dedicated layout for the Game Pad interface featuring responsive D-pad controls and behavior selection.
- `app-shared.js`: Core shared module managing websocket lifecycle, lease negotiation, theme/locale management, modal popups, audio feedback, and system power commands.
- `app-gamepad.js`: Handles directional D-pad touch/mouse inputs, game pad action buttons, and user behavior dropdown loading and execution.
- `app-agent.js`: Manages chat message streams, composer interactions, toggle state management for special robot buttons, and audio control toggles.

### User-created behavior execution via web controller
The web controller (`web_control.py`, `web_app.py`) introduces secure methods for listing and running user-saved behaviors:
- `list_user_behaviors(lease_id)`: Queries private user behaviors via `robot.assets.list_user(category)` and returns bounded metadata (`name`, `description`, `contains_motion`).
- `run_user_behavior(lease_id, name)`: Validates the requested behavior name against currently available user behaviors, stops active motion, and executes `robot.behavior.run`.
- Websocket messages: `user_behaviors_list` and `user_behavior_run`.

### Button styling and toggle states
- **Special Robot Controls (A / B / X / Y)** on Agent Interface:
  - Disabled / OFF state: black font and `2px solid black` border (`aria-pressed="false"`).
  - Enabled / ON state: white font and `2px solid white` border (`aria-pressed="true"`).
  - Button A toggles autonomous AI motion arming; Button B toggles temporary camera access; Button X triggers emergency stop; Button Y confirms and resumes movement.
- **Audio & Media Controls (Web Mic, Voice Input, Speech ON)**:
  - Disabled / OFF state: white font and 50% opacity white border (`border: 1px solid rgba(255, 255, 255, 0.5)`).
  - Enabled / ON state: blue font (`#5dade2`) and 50% opacity blue border (`border: 1px solid rgba(93, 173, 226, 0.5)`).

### IDE capability and menu restructuring
- `integrated.py`: `_BehaviorListAdapter` accepts an optional `source` argument (`"all"` | `"user"`).
- `interactive_tool.py`: Main interactive menu consolidated to options 1–5 and Q (Bluetooth Speaker Connection is option 5). Emergency stop injection via 'e' is removed.
- Full multi-language parity is maintained across all 4 locales (`en`, `ja`, `zh-TW`, `zh-CN`) with 146 translation keys each.
