...
## Refined guided setup — 23 September 2026

`ninjarobot onboard` opens an ASCII NINJAROBOT welcome screen and a staged guide.
New users see **1) Open setup tool** and **Q) Save and exit**. If standalone
hardware settings exist, the hardware entry menu also offers **2) Apply existing
settings for all modules**. This is one bulk action: it checks all saved hardware
settings, reuses valid modules, and guides missing or invalid required modules
through their existing tools. Optional Bluetooth pairing remains a separate step.

After a tool exits successfully, onboarding checks and displays its saved
settings. Press Enter to continue. After hardware setup, review one integrated
configuration preview and press Enter to save it. There are no typed YES, APPLY,
or camera/microphone CONSENT phrases. Obtain consent before selecting any media
capture test in a standalone tool. Servo setup still requires raised wheels.
Automatic configuration checks do not certify physical device operation.

`--simulation` prints an offline rehearsal without replacing real progress.
`--resume` rechecks saved configuration; simulated records cannot qualify for
real-hardware readiness. Ctrl+C or terminal EOF saves completed checkpoints.
Existing standalone hardware commands and manual configuration remain supported.

For Google Calendar MCP, use `ninjarobot onboard --step mcp` and select 2.
The wizard explains Google Cloud project/API setup, the consent screen and test
user, Desktop OAuth client creation, and credential-file placement. Select where
your browser runs. If it runs on a Mac, open another Mac Terminal and run:

```bash
ssh -N -o ExitOnForwardFailure=yes -L 127.0.0.1:8765:127.0.0.1:8765 YOUR_PI_USER@YOUR_PI_HOST
```

Replace the two placeholders with your usual Pi SSH login. Keep this terminal
open, then press Enter in onboarding and open its newly generated link on the
Mac. A quiet forwarding terminal is normal. The Mac's `127.0.0.1` is the Mac,
not the Pi: this forwarding connection delivers browser approval to the Pi.
If using a browser on the Pi itself, choose Pi desktop; no SSH tunnel is needed.

Onboarding displays waiting updates, acknowledges the callback, and validates
one bounded read-only events request through the external MCP server. It does
not print event contents. An empty calendar is a successful response. Retry
with a fresh link after a timeout; do not share authorization or callback URLs.
The external MCP setup does not require starting the Agent. The later native
Calendar instructions describe a separate integration.

Provider and Tavily replacement keys are validated before saving. Failed Calendar
reconnection removes its candidate credential and preserves the working entry.
Environment API keys still take precedence; update those in their environment.
ngrok is reported as configured until the launched service verifies its endpoint.
Skipping ngrok retains authenticated same-Wi-Fi HTTPS at final Agent launch.

See the [refinement walkthrough and manual tests](../../../../../docs/validation/onboarding_refinement_walkthrough_260923.md) for the complete
Mac connection sequence, expected results, and recovery instructions.
...
## Guided installation and onboarding — 22 September 2026

New installations can download the root bootstrap directly. The GitHub page URL
...
supported. Use `--dry-run` to preview and `./install.sh --check` after reboot.

The hardware installer adds `~/.local/bin/ninjarobot`. Reconnect over SSH if the
command is not yet on `PATH`, then run:

```bash
ninjarobot onboard
```

The wizard guides required `pi5buzzer`, `pi5disp`, `pi5vl53l0x`, `pi5servo`,
and `pi5camera` setup through the existing standalone tools and IDE hardware
lock. Servo setup requires raised wheels; camera and microphone setup require
privacy consent. Optional setup covers a Bluetooth speaker, `pi5mic` with local
whisper.cpp and the bundled `hey_Ninja.onnx` wake model, and one of Ollama,
Google, OpenAI, or Anthropic.

The optional-tools step configures Tavily Search, Google Calendar, and Notion as
external MCP providers, one at a time. The Calendar MCP preset is read-only and
separate from the native Calendar integration described later in this manual.
Secrets, OAuth tokens, and Calendar credentials use owner-only storage outside
`mcp.toml` and onboarding progress.

ngrok remains optional. Skipping it selects authenticated same-Wi-Fi HTTPS when
the user chooses **Start NinjaRobot Agent**. Choosing **Exit Onboarding** saves
progress and leaves the Agent stopped. Real-hardware launch remains blocked
until all required device settings pass automatic checks; simulation is the
default launch choice.
...
Terminal chat supports cursor editing. Enter sends, Shift+Enter or Alt+Enter
(Option+Enter on Mac) inserts a line, and Esc followed by Enter is the fallback
for terminals that cannot distinguish modifiers. Arrow keys navigate within the
current prompt; `/help` shows commands.

Follow the complete
[guided setup and manual-test walkthrough](../../../../../docs/validation/onboarding_install_chat_walkthrough_260922.md)
for component menu choices, account setup, expected results, and rollback.
...
For an already configured checkout, inspect service status and restart the
existing service when the robot is idle to load new code. Existing startup may
communicate with hardware. Preserve the private database backup before upgrade.
The walkthrough separates this operator step from hardware-free synthetic tests.
An installed wheel without the project checkout reports public help unavailable;
it does not search private folders or install a wiki engine.
...
The supported distance game is available without an enablement step. To change its
volume, add `[distance_game]` with `volume = 16` to the existing robot configuration
while the service is stopped. Volume accepts 1–32. Do not duplicate an existing
TOML section. Older private configurations may still contain `enabled = false`;
that field remains parseable for compatibility but no longer hides the supported
game. Restart using the existing service manager and account; never start a second
hardware owner. The full walkthrough explains both system service and CLI restart
methods. No new runtime dependency or boot service is needed.

`ninjarobot-agent game start --seconds 5` explicitly starts one game through the
running service. From another terminal, `ninjarobot-agent game stop` stops it and
`ninjarobot-agent game status` reads its state. Chat supports `/game start 5`,
`/game stop` and `/game status`. The web controller has matching controls.

The display, buzzer and distance sensor must be available. A Bluetooth speaker,
camera or microphone is not required. Keep wheels raised during first physical
acceptance and stay away from desk edges. The game never commands wheel movement.
Hold a hand 5–60 cm from the sensor; nearer produces higher short tones. Sessions
last 5–60 whole seconds, default 30. Invalid readings silence output and sustained
invalid readings end the game. Repeated games require new explicit requests.

`[interaction_variations] enabled = true` permits subtle, silent palette variations
for happy, laughing, excited and shy conversational faces. Named behaviors and
safety/privacy/startup/idle cues remain unchanged. Disable either option and
restart to return to the previous interaction settings.

A cancelled or timed-out sensor worker remains owned until it returns. The next
read, movement preparation, or game request can automatically wait for that work
to drain and check sensor health. This does not clear an explicit system stop or
a game output-cleanup fault: those still require successful full system recovery.
A permanently stuck worker requires operator/service recovery; do not bypass
checks or start another process against the device.
...
Follow these steps from the project root after the normal Lite installation and
hardware initialization below. The Agent must be stopped before changing audio
services; the setup's explicit start step carries the existing movement/capture
risk. For acceptance tests and rollback, use the
[full Phase 3 walkthrough](../../../../../docs/validation/refinement_phase3_walkthrough_260909.md).
...
checks. Follow the Lite account, lingering, version-specific WirePlumber and
service-environment steps above; pairing alone is insufficient.
...
### Step 3 — Download and run the project installer

Install Git, which the bootstrap uses to create a normal, updateable project
checkout:
...
project installer. Review the installer's displayed actions and confirmation
prompt. After installation, the checkout is at `~/NinjaRobotPi5`.

The existing manual workflow remains available when you need to inspect the
...
5. While a short guarded movement runs, place a flat target within 50 mm of the
   front sensor—confirm the wheels stop, the confusing face appears, Idle returns,
...
6. ✅ **A** enables spoken replies, **B** disables spoken replies
...
Long conversations retain their complete local transcript for the user and the
configured retention period. Before each model call, the Agent measures the fully
serialized request, including trusted safety instructions, tools, memory, and
conversation. If it exceeds the configured input budget, the Agent keeps the newest
complete user turns that fit and adds a trusted omission notice. An assistant tool
call and its result stay together. Older messages are not deleted, and bounded
long-term memory remains available for durable facts. Only a single current message
that cannot fit alongside required safety context is rejected; shorten that message
...
An ordinary `servo.stop`, including transition cleanup, `/disarm`, controller
loss, or replacing one movement with another, is also not a Level 2 Emergency
Stop. It immediately interrupts active wheel pulses through the priority stop lane
but does not create a system latch. Explicit Emergency Stop and unrecoverable
hardware failures continue to use Level 2 and require confirmed recovery.
...
## Web interface refinement and terminal menu consolidation: 18 September 2026

The web interface is restructured into two dedicated pages accessible from any modern browser:
- **Agent Interface** (`https://ninjarobotpi5.local:8443/` or `/agent`): Primary interface focusing on conversation, message history, audio/speech controls, and special robot controls. Includes a "Clear Message" action to clear visible chat elements without terminating or resetting the persistent chat session.
- **Game Pad** (`https://ninjarobotpi5.local:8443/gamepad`): Dedicated control interface featuring a touch D-pad, action buttons (A: Greeting, B: AI Camera preview, X: Emergency Stop, Y: Resume Movement), and a user-created behaviors selector with safety confirmation for motion actions.

Interface switching is managed via the top-left hamburger menu under "Control Interface", with user preferences persisted across visits. Multi-language parity is maintained across English, Japanese, Traditional Chinese, and Simplified Chinese (146 keys per locale).

Terminal IDE navigation (`ninjarobot-ide-tool interactive`) is consolidated to options 1 through 5 plus Q:
...
Emergency stop injection via single-character 'e' has been removed from terminal menus in favor of explicit menu choices and web/hardware stop triggers. The Agent CLI interactive menu is updated to options 1 through 14.
