...
## Guided external MCP setup — 22 September 2026

Run `ninjarobot onboard --step mcp` to select any combination of Tavily Search,
Google Calendar, and Notion. The wizard configures and validates one external
server at a time; the whole step can be skipped. Existing custom MCP entries are
preserved, and a reviewed preset refuses to replace a conflicting server ID.

The initial allowlists are deliberately read-only:

| Preset | Transport and authentication | Reviewed tools |
| --- | --- | --- |
| Tavily Search | Hosted HTTPS, bearer key from `SecretStore` | `tavily_search` |
| Google Calendar | Bundled external stdio MCP, private OAuth credential file | `list_today_events` |
| Notion | Official hosted HTTPS, SDK OAuth with private token storage | `notion-search`, `notion-fetch` |

The Calendar adapter is external to the Agent through MCP even though its small
server is packaged in this repository. It reuses `calendar_oauth.authorize` and
`GoogleCalendarBackend`, asks only for read-only event scope, resolves the
authorized primary calendar, and returns bounded titles/start/end/status. It
does not expose descriptions, attendees, conference links, or write tools. This
credential and server are separate from the native Calendar integration below.

Notion authorization uses a loopback callback on port 8766. From macOS, Alt is
called Option, but that keyboard naming is unrelated to OAuth: run the exact
displayed `ssh -L 8766:127.0.0.1:8766 USER@ROBOT` command in a local terminal,
then open the authorization URL. The Agent service will refresh stored tokens
but will fail closed instead of opening a new browser login. Reauthorize through
the onboarding MCP step.

Tool discovery must contain every exact allowlisted name before the preset is
saved. External content remains untrusted and cannot grant hardware authority.
Secrets never appear in `mcp.toml`, status output, progress, or logs. Use
`ninjarobot mcp list`, `mcp inspect SERVER`, and `mcp health SERVER` for manual
inspection; the existing `mcp add --preset tavily|notion` commands remain
...
for account preparation, validation, and credential recovery.
...
The working bundled project-help skill is version 2; older packages remain
version 1. Requirements name minimum Agent/tool versions, required capabilities,
optional fallbacks and effects. Effective access still comes from the enabled
tool catalog, user scope, request budget and deterministic policy.

See [packaging and examples](../../../../../docs/skill-templates/README.md).
Offline inspect/simulate checks package structure. Add --check-live to compare
with the already running service. Version-2 install and enable require that
catalog; they never start a service or install a missing provider.
The calendar/research/briefing templates are inactive future examples, not
working Phase 4 services.
...
### Distance game tools and skill

The existing IDE tool provider exposes `robot.game.distance.run`,
`robot.game.distance.stop` and `robot.game.distance.status`. No external MCP server
is needed. Run accepts only `duration_seconds` (integer 5–60, default 30) and `mode`
(`distance_tone`). Stop/status accept an empty object. Volume and wiring are
operator configuration, never model arguments. Run is non-idempotent: a new
request means a new physical experience; never retry automatically on uncertainty.

The bundled `distance-game` version-1 skill permits only those three tools.
Validate it with `ninjarobot-agent skill validate` and its bundled directory, or
select it explicitly with `ninjarobot-agent chat --skill distance-game`.
Natural-language requests still need a working model; `/game` commands do not.
No game-enable setting is required. Older configurations containing
`distance_game.enabled = false` remain parseable, but this retired compatibility
field does not hide the built-in supported game. Hardware health, resource
admission, bounded duration, stop, and system safety checks remain authoritative.

A tool result's outer status describes execution of the call. Read `data.state`
and `data.reason` to distinguish a finished game from cancellation, unavailable
readings or a fault. Current task evidence records that distinction. Never infer
that a person heard a tone, that a sensor prevents falls or that hardware
prototypes in the evaluation documents are supported devices.
...
## Phase 3 command-help skill — 12 September 2026

The bundled `robot-command-help` skill uses `command_help.search` to retrieve the
shared local command reference. Natural-language how-to questions about robot
features select it automatically when available. It explains commands and cannot
execute them, enable speech/microphone input or grant hardware permissions.
`/help TOPIC` and `/guide 1` through `/guide 5` are deterministic alternatives
that do not need a model call.

The model-facing `tasks.list` tool accepts `limit` (1–20, default 10), `after`,
`kind` (`all`, `reminder`, `request`) and `status` (`all`, `scheduled`, or an exact
task status). Return IDs and contents to the user. Follow `next_after` using the
same filters only when needed; do not concatenate every page into a model message.
Task approval still requires the user's direct confirmation command.

See the [current walkthrough](../../../../../docs/validation/refinement_phase3_followup_walkthrough_260912.md).
This new source is readable directly; wiki ingestion and review remain skipped
at the owner's request. The older sections below describe the existing extension
contracts, which are preserved.
...
the selected conversation provider. No calendar-write tools were added at that earlier checkpoint.
...
This is the separate legacy/custom MCP example. For normal setup, use the guided
Google Calendar authorization at the start of this manual. That path registers
the built-in connection and supports reviewed writes. The example below keeps
a separate read-only token and does not grant event creation. Do not overwrite
the repaired compatibility wrapper unless intentionally restoring this legacy flow.
...
## Web interface refinement and tool navigation: 18 September 2026

The web controller provides dual operational surfaces:
- **Game Pad View** (`/gamepad`): For direct directional steering and execution of built-in or user-created behaviors with motion confirmation.
- **Agent View** (`/agent`): For natural language dialogue, task planning, and information queries.

Chat commands remain fully accessible from the Agent composer:
- Guided checks: `/guide` or `/guide [1-5]`
- Distance sensor game: `/game start [distance]`, `/game stop`, `/game status`
- Task reminders: `/tasks`
- Command help: `/help [topic]`

The hamburger menu provides immediate navigation between views without losing connection or chat session context. When user-created behaviors include movement, the frontend requires explicit user confirmation before dispatching execution to the robot.
