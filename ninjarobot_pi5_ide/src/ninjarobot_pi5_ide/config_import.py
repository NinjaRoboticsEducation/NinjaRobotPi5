...
def robot_config_to_toml(config: RobotConfig) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            "[memory]",
            f"enabled = {_toml(config.memory.enabled)}",
            f"conversation_retention_days = {config.memory.conversation_retention_days}",
            (f"failed_behavior_retention_days = {config.memory.failed_behavior_retention_days}"),
            f"failed_behavior_cap = {config.memory.failed_behavior_cap}",
            f"retrieval_limit = {config.memory.retrieval_limit}",
            f"retrieval_character_budget = {config.memory.retrieval_character_budget}",
            f"face_data_directory = {_toml(config.memory.face_data_directory)}",
            "",
            "[voice_input]",
            f"enabled = {_toml(config.voice_input.enabled)}",
            f"model_resource = {_toml(config.voice_input.model_resource)}",
            f"inference_framework = {_toml(config.voice_input.inference_framework)}",
            f"wake_threshold = {config.voice_input.wake_threshold}",
            f"vad_enabled = {_toml(config.voice_input.vad_enabled)}",
            f"wake_vad_threshold = {config.voice_input.wake_vad_threshold}",
            f"silence_rms_threshold = {config.voice_input.silence_rms_threshold}",
            (f"noise_suppression_enabled = {_toml(config.voice_input.noise_suppression_enabled)}"),
            f"max_command_seconds = {config.voice_input.max_command_seconds}",
            f"silence_stop_seconds = {config.voice_input.silence_stop_seconds}",
            f"cooldown_seconds = {config.voice_input.cooldown_seconds}",
            f"frame_milliseconds = {config.voice_input.frame_milliseconds}",
            ...
            f"executable = {_toml(config.remote_access.executable)}",
            f"authtoken_env = {_toml(config.remote_access.authtoken_env)}",
            f"pairing_secret_env = {_toml(config.remote_access.pairing_secret_env)}",
            f"session_secret_env = {_toml(config.remote_access.session_secret_env)}",
            f"remote_header_secret_env = {_toml(config.remote_access.remote_header_secret_env)}",
            f"local_upstream = {_toml(config.remote_access.local_upstream)}",
            f"connect_timeout_seconds = {config.remote_access.connect_timeout_seconds}",
            f"retry_initial_seconds = {config.remote_access.retry_initial_seconds}",
            f"retry_max_seconds = {config.remote_access.retry_max_seconds}",
            f"retry_limit = {config.remote_access.retry_limit}",
            f"health_interval_seconds = {config.remote_access.health_interval_seconds}",
            f"pairing_lifetime_seconds = {config.remote_access.pairing_lifetime_seconds}",
            f"session_lifetime_seconds = {config.remote_access.session_lifetime_seconds}",
            "",
            "[onboarding]",
            f"enabled = {_toml(config.onboarding.enabled)}",
            f"qr_error_correction = {_toml(config.onboarding.qr_error_correction)}",
            f"qr_border_modules = {config.onboarding.qr_border_modules}",
            f"greeting_behavior = {_toml(config.onboarding.greeting_behavior)}",
            f"idle_behavior = {_toml(config.onboarding.idle_behavior)}",
            f"connection_timeout_seconds = {config.onboarding.connection_timeout_seconds}",
            "",
            "[deployment]",
            f"auto_start_enabled = {_toml(config.deployment.auto_start_enabled)}",
            f"web_poweroff_enabled = {_toml(config.deployment.web_poweroff_enabled)}",
            f"systemd_unit_name = {_toml(config.deployment.systemd_unit_name)}",
            f"restart_policy = {_toml(config.deployment.restart_policy)}",
            f"poweroff_helper = {_toml(config.deployment.poweroff_helper)}",
        ]
    )
